...
class Generic {
 public:
  ...
  using VariantType = std::variant<bool, int, int64_t, double, std::string, Object,
                                   Array, std::nullopt_t>;
  using ReflectionType = std::optional<
      std::variant<bool, int, int64_t, double, std::string, Object, Array>>;
  ...
  /// Casts the underlying value to an int64_t or returns an rfl::Error, if the
  /// underlying value is not an int64_t.
...
/// Casts the underlying value to an int64_t or returns an rfl::Error, if the
/// underlying value is not an int64_t.
inline Result<int64_t> to_int64(const Generic& _g) noexcept {
  return _g.to_int64();
}
